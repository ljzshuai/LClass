...
namespace LJZ{
	...
	public:
		...
		bool start_once(unsigned long long MilliSecond, std::function<void()> func);
		bool start_continue(unsigned long long MilliSecond, std::function<void()> func);

		bool stop(); //for start_continue
	private:
		...
		std::atomic<bool> can_start_;
		std::atomic<bool> be_stoped_;
		std::mutex mutex_;
